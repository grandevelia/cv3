...
# python ./eval.py ./configs/voc_fcos.yaml ../logs/

# import pickle
# with open('../../tmp/cv3/code/objs.pkl', 'rb') as f:
#    targets, center_sampling_radius, strides, points, reg_range, cls_logits, reg_outputs, ctr_logits = pickle.load(f)

# import pickle
# with open('../../tmp/cv3/code/inf.pkl', 'rb') as f:
...
class FCOS(nn.Module):
    ...
    * All losses are normalized by the number of positive points (Eq 2 in the paper)
    ...
    def compute_loss(
        ...
        point_xs = all_points[:, 1].unsqueeze(1) #Unsqueeze to make shape compatible with targets
        point_ys = all_points[:, 0].unsqueeze(1)
        ...
        for target_idx, target in enumerate(targets): #This loops through the batch
            ...
            center_xs = target_centers[:, 0]
            center_ys = target_centers[:, 1]
            # Ensure point is within a sub-box
            center_x_dists = (all_points[:, 1, None] - center_xs[None, :]).abs() #N_targes * n_pixels
            center_y_dists = (all_points[:, 0, None] - center_ys[None, :]).abs()
            center_dists = torch.cat([center_y_dists.unsqueeze(-1), center_x_dists.unsqueeze(-1)], dim=-1)
            max_center_offsets = (
                center_dists
            ...
            point_offsets = torch.stack([ # Tensor of shape [Nx*Ny, Ntargets, , 4]
            ...
            points_in_bbs = point_offsets.min(dim=-1).values > 0 #We only care about the lowest distance, since any < 0 disqualifies the point. 
            #If any of the above distances is < 0, the point is disqualified
            #Ensure points in regression range
            point_offsets = point_offsets.max(dim=-1).values
            max_point_offsets = center_dists.max(dim=-1).values
            min_point_offsets = center_dists.min(dim=-1).values
            lower_bound = point_reg_ranges[:, 0, None]
            upper_bound = point_reg_ranges[:, 1, None]
            points_in_reg_range = (
                (min_point_offsets > lower_bound) & 
                (max_point_offsets < upper_bound)
            )
            all_pos_points = points_in_radius & points_in_bbs & points_in_reg_range #N points x n_targets
            ...
            point_cls_labels = target["labels"][min_idxs.clip(min=0)]
            point_cls_labels[min_idxs < 0] = -1 #This allows finding FG mask without storing another array
            point_box_targets = target_boxes[min_idxs.clip(min=0)] #Don't need to set now since we will use FG mask later
            all_cls_labels.append(point_cls_labels)
            all_box_targets.append(point_box_targets)
                            
        all_cls_labels = torch.stack(all_cls_labels) #[bs, nx*ny]
        ...
        box_target_centers = (all_box_targets[:, :, :2] + all_box_targets[:, :, 2:])/2 #point coordinates
        center_xs, center_ys = box_target_centers.unbind(dim=-1) #point coordinates
        reg_strided = reg_outputs * point_strides[None, :, None]
        reg_lefts = center_xs - reg_strided[:, :, 0] #point coordinates
        reg_tops = center_ys - reg_strided[:, :, 1] #point coordinates
        reg_rights = center_xs + reg_strided[:, :, 2] #point coordinates
        reg_bottoms = center_ys + reg_strided[:, :, 3] #point coordinates
        reg_coords = torch.stack([reg_lefts, reg_tops, reg_rights, reg_bottoms], dim=-1) #point coordinates
        ...
        #centeredness loss
        point_l_dists = (all_points[:, 1].unsqueeze(0) - all_box_targets[..., 0])
        point_t_dists = (all_points[:, 0].unsqueeze(0) - all_box_targets[..., 1])
        point_r_dists = (all_box_targets[..., 2] - all_points[:, 1].unsqueeze(0))
        point_b_dists = (all_box_targets[..., 3] - all_points[:, 0].unsqueeze(0))
        point_dists = torch.stack([point_l_dists, point_t_dists, point_r_dists, point_b_dists], dim=-1)

        #normalize by target size to help training because Abrar said it was cool & pytorch did it
        #target_widths = all_box_targets[..., 2] - all_box_targets[..., 0]
        #target_heights = all_box_targets[..., 3] - all_box_targets[..., 1]
        #dim_sizes = torch.stack([target_widths, target_heights, target_widths, target_heights], dim=-1)
        #point_dists = point_dists / dim_sizes

        horizontal_offsets = point_dists[..., [0, 2]]
        vertical_offsets = point_dists[..., [1, 3]]
    ...
    * Output labels needed to be offseted by +1 to compensate for the input label transform
    ...
    def inference(
        ...
        for i, image_shape in enumerate(image_shapes): #Loop through the batch
            ...
            for level_idx, level_points in enumerate(points): #loop through the pyramid levels
                ...
                object_scores = torch.sqrt(torch.sigmoid(level_cls) * torch.sigmoid(level_ctr)).flatten()
                ...
                keep_idxs = object_scores > score_thresh
                object_scores = object_scores[keep_idxs]
                ...
                #Since the flattened score array hits a new point every n_classes,
                # we can recover original point indices with floor division,
                # and class labels with modulus
                top_point_idxs = torch.div(topk_idxs, num_classes, rounding_mode="floor") 
                top_labels = topk_idxs % num_classes
                #Make sure
                top_points = level_points[top_point_idxs]
                top_regression_preds = level_reg[top_point_idxs]
                box_lefts = top_points[:, 1] - top_regression_preds[:, 0]
                box_tops = top_points[:, 0] - top_regression_preds[:, 1]
                box_rights = top_points[:, 1] + top_regression_preds[:, 2]
                box_bottoms = top_points[:, 0] + top_regression_preds[:, 3]
                #Coordinates of predicted boxes for each point
                kept_boxes = torch.stack([box_lefts, box_tops, box_rights, box_bottoms], dim=-1)
                all_boxes.append(kept_boxes)
                all_scores.append(object_scores)
                all_labels.append(top_labels)

            #b) combine object candidates (already combined)
            top_boxes = torch.cat(all_boxes, dim=0)
            top_scores = torch.cat(all_scores, dim=0)
            top_labels = torch.cat(all_labels, dim=0)

            #c) Remove boxes outside image boundaries
            outside_boxes = (
                (top_boxes[:, 0] < 0) | 
                (top_boxes[:, 1] < 0) | 
                (top_boxes[:, 2] > image_shape[1]) | 
                (top_boxes[:, 3] > image_shape[0])
            )
            top_boxes = top_boxes[~outside_boxes]
            top_scores = top_scores[~outside_boxes]
            top_labels = top_labels[~outside_boxes]

            #d) run NMS to remove any duplicated boxes & keep the top-k boxes after NMS
            nms_keeps = batched_nms(top_boxes, top_scores, top_labels, nms_thresh)[:detections_per_img]
